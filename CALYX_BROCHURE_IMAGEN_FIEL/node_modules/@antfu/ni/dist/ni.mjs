...
import { A as require_prompts, T as formatPackageWithUrl, h as getCatalog, i as runCli, j as require_ini, l as parseNi, o as getCommand, w as exclude } from "./src-0UuqNAsQ.mjs";
...
//#endregion
//#region src/catalog/range-prefix.ts
var import_ini = /* @__PURE__ */ __toESM(require_ini(), 1);
const DEFAULT_PREFIX = "^";
function findConfigFiles(cwd, fileName) {
	const found = [];
	let dir = path.resolve(cwd);
	while (true) {
	...
	if (!found.includes(homeConfig) && fs.existsSync(homeConfig)) found.push(homeConfig);
	return found;
}
function isTrue(value) {
	return value === true || value === "true";
}
function npmRangePrefix(cwd) {
	if (isTrue(process.env.npm_config_save_exact)) return "";
	if (process.env.npm_config_save_prefix !== void 0) return process.env.npm_config_save_prefix;
	for (const filePath of findConfigFiles(cwd, ".npmrc")) {
		let parsed;
		try {
			parsed = import_ini.default.parse(fs.readFileSync(filePath, "utf-8"));
		} catch {
			continue;
		}
		if (isTrue(parsed["save-exact"])) return "";
		if (typeof parsed["save-prefix"] === "string") return parsed["save-prefix"];
	}
	return DEFAULT_PREFIX;
}
function yarnRangePrefix(cwd) {
	const fromEnv = process.env.YARN_DEFAULT_SEMVER_RANGE_PREFIX;
	if (fromEnv !== void 0) return fromEnv;
	for (const filePath of findConfigFiles(cwd, ".yarnrc.yml")) {
		let json;
		try {
			json = parsePnpmWorkspaceYaml(fs.readFileSync(filePath, "utf-8")).toJSON();
		} catch {
			continue;
		}
		if (typeof json.defaultSemverRangePrefix === "string") return json.defaultSemverRangePrefix;
	}
	return DEFAULT_PREFIX;
}
/**
...
*/
function getRangePrefix(agent, cwd) {
	if (agent === "yarn@berry") return yarnRangePrefix(cwd);
	if (agent === "pnpm") return npmRangePrefix(cwd);
	return DEFAULT_PREFIX;
}
//#endregion
//#region src/catalog/spec.ts
...
function splitPackagesAndFlags(args) {
	...
	else packages.push(parsePackageSpec(arg));
	return {
		packages,
		flags
	};
...
async function resolveVersion(pkg, rangePrefix) {
	if (pkg.spec && isVerbatimSpec(pkg.spec)) return pkg.spec;
	return `${rangePrefix}${(await getLatestVersion(pkg.raw)).version}`;
}
function hasRemainingNewPackages(provider, config, packages, currentIndex) {
	for (let i = currentIndex + 1; i < packages.length; i++) if (!provider.findPackage(config, packages[i].name)) return true;
	return false;
}
async function handleCatalogInstall(agent, args, ctx) {
	const catalogEnabled = await getCatalog();
	if (!catalogEnabled) return void 0;
	...
	for (let i = 0; i < packages.length; i++) {
		...
		if (existing) {
			if (!ctx?.programmatic) {
				...
				const note = pkg.spec && pkg.spec !== cataloged ? `→ found in ${existing.name} catalog (${cataloged}, ignoring ${pkg.spec})` : `→ found in ${existing.name} catalog`;
		...
		if (catalogName) {
			...
			if (!ctx?.programmatic) console.log(`${styleText("green", "+")} ${styleText("cyan", pkg.name)} ${styleText("dim", `→ ${catalogName} catalog (${version})`)}`);
			catalogEntries.push({
				name: pkg.name,
				catalogRef: getCatalogRef(catalogName)
			});
		} else skippedPackages.push(pkg.raw);
	}
