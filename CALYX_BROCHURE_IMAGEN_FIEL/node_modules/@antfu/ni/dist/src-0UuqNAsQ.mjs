...
function readRcFile(path) {
	...
	try {
		...
		console.warn(`[ni] cannot read ${path}: ${error.message}`);
		return {};
	}
	let parsed;
	try {
		parsed = import_ini.default.parse(contents);
	} catch (error) {
		console.warn(`[ni] cannot parse ${path}: ${error.message}`);
		return {};
	}
	const known = {};
	for (const [key, value] of Object.entries(parsed)) {
		...
		console.warn(`[ni] unknown option "${key}" in ${path}${suggestion ? ` (did you mean "${suggestion}"?)` : ""}`);
	}
	return known;
}
let config;
async function getConfig() {
	if (!config) {
		config = {
			...defaultConfig,
			...fs.existsSync(rcPath) ? readRcFile(rcPath) : null
		};
		...
		if (process$1.env.NI_CATALOG !== void 0) config.catalog = process$1.env.NI_CATALOG === "existing" ? "existing" : process$1.env.NI_CATALOG !== "false";
...
const parseNd = ((agent, args) => {
	if (agent === "pnpm" || agent === "nub" || agent === "aube") args = args.map((i) => i === "-c" ? "--check" : i);
...
//#endregion
export { require_prompts as A, cmdExists as C, remove as D, limitText as E, terminalLink as O, CLI_TEMP_DIR as S, formatPackageWithUrl as T, getDefaultAgent as _, UnsupportedCommand as a, getUseSfw as b, parseNd as c, parseNr as d, parseNun as f, getConfig as g, getCatalog as h, runCli as i, require_ini as j, writeFileSafe as k, parseNi as l, serializeCommand as m, getCliCommand as n, getCommand as o, parseNup as p, run as r, parseNa as s, src_exports as t, parseNlx as u, getGlobalAgent as v, exclude as w, detect$1 as x, getRunAgent as y };
