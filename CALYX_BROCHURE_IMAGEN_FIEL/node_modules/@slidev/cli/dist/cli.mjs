...
import { c as getThemeMeta, d as loadSetups, f as updateFrontmatterPatch, l as resolveTheme, o as resolveOptions, s as parser, t as createServer, u as version } from "./serve-Be4VBJJX.mjs";
...
cli.command("* [entry]", "Start a local server for Slidev", (args) => commonOptions(args).option("port", {
	...
	async function initServer() {
		...
		let publicIp;
		if (remote) publicIp = await resolvePublicIp();
...
cli.command("build [entry..]", "Build hostable SPA", (args) => exportOptions(commonOptions(args)).option("out", {
	...
	const { build } = await import("./build-BUQLxoIx.mjs");
...
cli.command("mcp [entry]", "Start an MCP (Model Context Protocol) server over stdio for AI agents to inspect and edit the slides", (args) => commonOptions(args).strict().help(), async ({ entry }) => {
	const { startMcpStdioServer } = await import("./stdio-SXW6hFKc.mjs");
...
cli.command("export [entry..]", "Export slides to PDF", (args) => exportOptions(commonOptions(args)).strict().help(), async (args) => {
	...
	const { exportSlides, getExportOptions } = await import("./export-LauAJGMx.mjs");
...
cli.command("export-notes [entry..]", "Export slide notes to PDF", (args) => args.positional("entry", {
	...
	const { exportNotes } = await import("./export-LauAJGMx.mjs");
	const candidatePort = await getPort(12445);
	for (const entryFile of entry) {
