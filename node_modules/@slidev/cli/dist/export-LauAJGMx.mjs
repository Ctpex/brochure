...
async function exportSlides({ port = 18724, total = 0, range, format = "pdf", output = "slides", slides, base = "/", timeout = 3e4, wait = 0, dark = false, routerMode = "history", width = 1920, height = 1080, withClicks = false, executablePath = void 0, withToc = false, perSlide = false, scale = 1, waitUntil, omitBackground = false }) {
	...
	try {
		...
		else if (format === "pptx-editable") await genPagePptxEditable();
	...
	async function genPagePptxEditable() {
		if (perSlide) throw new Error("[slidev] `--per-slide` is not supported with `--format pptx-editable`");
		const { exportPptxEditable, reportEditableExport } = await import("./pptx-D2UE590V.mjs");
		const result = await exportPptxEditable({
			page,
			slides,
			width,
			height,
	...
	function addPdfMetadata(pdf) {
		...
		if (titleSlide?.frontmatter?.keywords) {
			if (Array.isArray(titleSlide?.frontmatter?.keywords)) pdf.setKeywords(titleSlide?.frontmatter?.keywords);
