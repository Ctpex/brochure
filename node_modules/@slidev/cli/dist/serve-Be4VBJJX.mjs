...
function applyNotesAutoRuby(md, notesAutoRuby) {
	const keys = Object.keys(notesAutoRuby).sort((a, b) => b.length - a.length).map((i) => RE_WORD_CHARS_ONLY.test(i) ? `\\b${i}\\b` : i.replace(RE_REGEXP_CHARS, "\\$&"));
	if (!keys.length) return md;
	return md.replace(new RegExp(`(${keys.join("|")})`, "g"), (match) => {
...
function createSlidesLoader(options, serverOptions) {
			...
			server.middlewares.use(async (req, res, next) => {
				...
				if (req.method === "GET") {
					...
					if (body.frontmatterRaw != null) {
						if (body.frontmatterRaw.trim() === "") slide.source.frontmatterDoc = slide.source.frontmatterStyle = void 0;
						else {
	...
	function renderNote(text = "") {
		...
		const md = text.replace(/\[click(?::(\d+))?\]/gi, (_, count = 1) => {
			clickCount += Number(count);
			return `<span class="slidev-note-click-mark" data-clicks="${clickCount}"></span>`;
		});
		return notesMd.render(applyNotesAutoRuby(md, notesAutoRuby));
	}
...
//#endregion
//#region ../../node_modules/.pnpm/@hedgedoc+markdown-it-plugins@2.1.4_patch_hash=49e14003b6caa0b7d164cbe71da573809d375bab_18adf40ca8d248c8719aefb5f4dca995/node_modules/@hedgedoc/markdown-it-plugins/dist/esm/image-size/specialCharacters.js
...
//#endregion
//#region ../../node_modules/.pnpm/markdown-it@14.3.2/node_modules/markdown-it/lib/token.mjs
/**
* class Token
**/
/**
...
//#endregion
//#region ../../node_modules/.pnpm/@hedgedoc+markdown-it-plugins@2.1.4_patch_hash=49e14003b6caa0b7d164cbe71da573809d375bab_18adf40ca8d248c8719aefb5f4dca995/node_modules/@hedgedoc/markdown-it-plugins/dist/esm/task-lists/index.js
...
async function applySlidePatch(data, no, patch) {
	...
	if (patch.frontmatterRaw != null) {
		if (patch.frontmatterRaw.trim() === "") source.frontmatterDoc = source.frontmatterStyle = void 0;
		else {
